...
export const CrearCitaTutoriaScreen = ({navigation}: NavigationProps) => {
  ...
  const [customTutoresIcon, setCustomTutoresIcon] = useState(
    <Icon
      name="account"
      color={
        colorSchema === 'dark' ? colores.Cool_Gray_5_C : colores.Pantone_382_C
      }
      size={30}
    />,
  );
  const [customCoursesIcon, setCustomCoursesIcon] = useState(
    <Icon
      name="library"
      color={
        colorSchema === 'dark' ? colores.Cool_Gray_5_C : colores.Pantone_382_C
  ...
  //TUTOR IMAGE
  const [tutorPhoto, setTutorPhoto] = useState<ImageSourcePropType>(
    require('@src/resources/Images/male-placeholder.jpeg'),
  );
  //TUTOR
  ...
  const handleErrorResp = (code: string | undefined): void => {
    ...
    if (code === 'ImageNotFound')
      return showMessage(
        'Ocurrio una excepcion trayendo la imagen del docente',
        'info',
      );
  ...
  const onSubmitFirstPart = async () => {
    ...
    try {
      ...
      setTutorInfo(currentTutorInfo => {
        ...
        return JSON.stringify(currentTutorInfo) !==
          JSON.stringify(tutorInfoResp)
          ? tutorInfoResp
          : currentTutorInfo;
      });
      ...
      setTutorPhoto(currentTutorPhoto => {
        // Only update state if new value differs from current
        return currentTutorPhoto !== tutorPhotoResp
          ? tutorPhotoResp
          : currentTutorPhoto;
      });
  ...
  const onSubmit = async (data: TFormData) => {
    ...
    try {
      ...
      if (!insertResp.success) {
        setClickInsertTutoria(false); //active loader
        setConfModalVisible(false);
        showMessage(insertResp.message, 'danger');
        return;
      }
      setClickInsertTutoria(false); //active loader
      setConfModalVisible(false);
      showMessage(insertResp.message, 'success');
      ...
      setConfModalVisible(false);
      showMessage(
        'Hubo un error al insertar la tutoria, porfavor intente mas tarde',
        'warning',
      );
    }
  };
              ...
              <View
                ...
                <Text
                  style={{
                    ...
                    fontSize: 14,
                    color:
                      colorSchema === 'dark' ? 'white' : colores.Pantone_382_C,
                  }}>
                ...
                <Text
                  style={{
                    fontWeight: colorSchema === 'dark' ? '500' : 'bold',
                    fontSize: 14,
                    color:
                      colorSchema === 'dark' ? 'white' : colores.Pantone_382_C,
                  }}>
          ...
          <ScrollView indicatorStyle="white">
            ...
            <View
              ...
              <View style={styles.buttonGuardarContentChild}>
                <Pressable
                  disabled={
                    !isOneEmpty(fecha_tutoria, day, franja) ? false : true
                  }
                  onPress={() => {
                  ...
                  style={{
                    ...
                    backgroundColor: !isOneEmpty(fecha_tutoria, day, franja)
                      ? colores.Pantone_383_C
            ...
            <View style={{}}>
              ...
              <View
                ...
                <View
                  ...
                  <View style={{width: '50%'}}>
                    <Text style={{fontWeight: '400'}}>
                      {Capitalize(
                        coursesItems?.find(e => e.value === id_course)?.label,
                      )}
                    </Text>
                  </View>
                </View>
                ...
                <View
                  ...
                  <View style={{width: '50%'}}>
                    <Text style={{fontWeight: '400'}}>{tema}</Text>
                  </View>
                </View>
                {/* Dia */}
                <View
                  ...
                  <View style={{width: '50%'}}>
                    <Text style={{fontWeight: '400'}}>{Capitalize(day)}</Text>
                  </View>
                </View>
                {/* Fecha */}
                <View
                  ...
                  <View style={{width: '50%'}}>
                    <Text style={{fontWeight: '400'}}>{fecha_tutoria}</Text>
                  </View>
                </View>
                {/* Franja */}
                <View
                  ...
                  <View style={{width: '50%'}}>
                    <Text style={{fontWeight: '400'}}>
                      {franjasItems?.find(e => e.value === franja)?.label}
                    </Text>
                  </View>
                </View>
                {/* Modalidad */}
              ...
              <View
                ...
                {errors.celular ? (
                  <View style={{width: '100%', marginLeft: width * 0.01}}>
                    <Text style={{color: colores.danger, fontWeight: '500'}}>
                      {errorHandlerCelular(errors.celular?.type)}
                    </Text>
                  </View>
                ) : (
                  <></>
  ...
  //VALIDATE SUBMIT
  const validateButtonSubmit = !isOneEmpty(
    id_course,
    day,
    franja,
    fecha_tutoria,
